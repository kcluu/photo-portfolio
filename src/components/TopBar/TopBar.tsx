      ...
      {categories.map((category) => {
        ...
        return (
          <button
            key={category}
            type="button"
            className={`top-bar__category-tab ${isActive ? "top-bar__category-tab--active" : ""}`}
            onClick={() => onSelectCategory(category)}
          >
            <span className="top-bar__category-paren" aria-hidden="true">
              (
            </span>
    ...
    <nav className="top-bar__nav">
      {navLinks.map((link) => (
        <a key={link} href={`#${link.toLowerCase()}`} className="top-bar__link">
          <span className="top-bar__link-paren" aria-hidden="true">
            (
          </span>
          {link}
          <span className="top-bar__link-paren" aria-hidden="true">
      ...
      <button
        type="button"
        className={`top-bar__light-switch ${lightsOn ? "top-bar__light-switch--on" : ""}`}
        onClick={onToggleLights}
        aria-pressed={lightsOn}
        aria-label={lightsOn ? "Turn lights off" : "Turn lights on"}
      >
        <svg
          className="top-bar__light-switch-icon"
          viewBox="0 0 24 24"
          width="18"
          height="18"
          fill="none"
        ...
        >
          ...
          <g className="top-bar__light-switch-rays">
            <path d="M12 0v2" />
